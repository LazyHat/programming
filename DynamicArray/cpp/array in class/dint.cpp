...
dint::dint(const size_t &amountOfElements)
{
...
dint &dint::create(const size_t &amountOfElements)
{
    ...
    if (NULL == this->value)
    {
        throw dintExeption("dint::create(const size_t &amountOfElements): bad_alloc");
    }
    this->_size = amountOfElements;
    return *this;
}
...
dint &dint::resize(const size_t &newsize)
{
    if (nullptr == this->value)
    {
        this->create(newsize);
    ...
    else
    {
        ...
        else if (newsize > this->size())
        {
            dint temp(*this);
            free(this->value);
            this->value = (int *)calloc(newsize, sizeof(int));
            this->_size = newsize;
        ...
        else
        {
            dint temp(*this);
            free(this->value);
            this->value = (int *)malloc(newsize * sizeof(int));
            this->_size = newsize;
            for (int i = 0; i < this->size(); i++)
            {
                this->value[i] = temp.value[i];
